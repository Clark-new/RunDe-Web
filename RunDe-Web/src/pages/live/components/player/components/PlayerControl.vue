...
export default {
  ...
  computed: {
    ...
    playerControlWrapper () {
      return this.$refs.PlayerControlWrapper
    }
  },
  watch: {
    status () {
  ...
  methods: {
    ...
    init () {
      ...
      const target = this.playerControlWrapper
    ...
    addEvents () {
      ...
      this.bus.$on('exit-fulls-screen', () => {
        if (this.fullscreen.isWebFullScreen()) {
    ...
    onSizeSensor () {
      this.unbind = bind(this.playerControlWrapper, element => {
        this.bus.$emit('size-sensor', element)
      })
    }
  },
  mounted () {
  ...
  beforeDestroy () {
    if (this.unbind) {
      clear(this.playerControlWrapper)
      this.unbind = null
    }
  }
}
